...
public class Cruise extends Sprite {
	...
	public void move() {
		...
		if(CommandCenter.getInstance().getMovFoes().size()!=0) {
			Movable asteroid = CommandCenter.getInstance().getMovFoes().get(0);

			if (getExpire() < MAX_EXPIRE - 5) {
				double diffx = asteroid.getCenter().getX() - getCenter().getX();
				double diffy = asteroid.getCenter().getY() - getCenter().getY();
				setDeltaX((getDeltaX() * 0.993 + diffx * 0.07) * 0.95);
				setDeltaY((getDeltaY() * 0.993 + diffy * 0.07) * 0.95);
				double tan = (double) (asteroid.getCenter().getY() - getCenter().getY() / (asteroid.getCenter().getX() - getCenter().getX()));
				int degree = (int) (Math.atan(tan) * 360 / 2 / Math.PI);
				double rotate = 0.9;
				setOrientation((int) (getOrientation() * rotate + degree * (1 - degree)));
				setDeltaX(FIRE_POWER * Math.cos(Math.toRadians(getOrientation())));
				setDeltaX(FIRE_POWER * Math.sin(Math.toRadians(getOrientation())));
