...
public class Game implements Runnable, KeyListener {
	...
	private final int PAUSE = 80, // p key
			QUIT = 81, // q key
			LEFT = 65, // rotate left; left arrow
			RIGHT = 68, // rotate right; right arrow
			UP = 87, // thrust; up arrow
			START =83, // s key//83
			FIRE = 32, // space key//32
			MUTE = 77, // m-key mute
			FIRE_LEFT = 38, //fire up, up arrow
			rightBilnk = 39, // move right, right arrow
			leftBlink = 37, // move left, up arrow
			...
			for (Movable movFoe : CommandCenter.getInstance().getMovFoes()) {
				...
				if (pntFriendCenter.distance(pntFoeCenter) < (nFriendRadiux + nFoeRadiux)) {
					...
					if ((movFriend instanceof Falcon) ){
						if (!CommandCenter.getInstance().getFalcon().getProtected()&&((Falcon) movFriend).getShield()<=0){
							...
							((Falcon) movFriend).setShield(((Falcon) movFriend).getnShield()-100);
						}
					}
					//not the falcon
					else {
			...
			for (Movable movFloater : CommandCenter.getInstance().getMovFloaters()) {
				...
				if (pntFalCenter.distance(pntFloaterCenter) < (nFalRadiux + nFloaterRadiux)) {
					if(movFloater instanceof NewShipFloater){
						switch (((NewShipFloater)movFloater).getType()){
							case addlife:CommandCenter.getInstance().setNumFalcons(CommandCenter.getInstance().getNumFalcons()+1);
							case shield:CommandCenter.getInstance().getFalcon().setShield(100);
						}
					}
					if(movFloater instanceof Boom){
	...
	private void killFoe(Movable movFoe) {
		...
		CommandCenter.getInstance().getOpsList().enqueue(new Debris(40,movFoe.getCenter()), CollisionOp.Operation.ADD);
        CommandCenter.getInstance().setScore(CommandCenter.getInstance().getScore()+movFoe.getScore());
	...
	private void spawnNewShipFloater() {
		...
		if (nTick % (SPAWN_NEW_SHIP_FLOATER - nLevel * 7) == 0) {
			CommandCenter.getInstance().getMovFloaters().add(new NewShipFloater());
	...
	public void spawnNewUfo(){
		if(R.nextInt(200) > 198){
			CommandCenter.getInstance().getMovFoes().add(new Ufo());
			CommandCenter.getInstance().getOpsList().enqueue(new Ufo(),CollisionOp.Operation.ADD);
		}
	}
    public void spawnNewBoom(){
		if(R.nextInt(300)>298){
			CommandCenter.getInstance().getMovFloaters().add(new Boom());
			CommandCenter.getInstance().getOpsList().enqueue(new Boom(),CollisionOp.Operation.ADD);
		}
	}
	...
	public void keyPressed(KeyEvent e) {
		...
		if (fal != null) {
			...
			case RIGHT:
				...
				break;
				case FIRE_DOWN:
					CommandCenter.getInstance().getOpsList().enqueue(new EnergeCannon(fal), CollisionOp.Operation.ADD);
			...
			// case NUM_ENTER:
				...
				case absoluteweapon:
					if(fal.getShield()>0){
						CommandCenter.getInstance().getOpsList().enqueue(new absoluteweapon(fal),CollisionOp.Operation.ADD);
						Sound.playSound("laser.wav");
						break;
					}
			default:
